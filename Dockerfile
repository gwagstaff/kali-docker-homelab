...
RUN apt -y update && \
    apt install --yes --no-install-recommends vim telnet nmap metasploit-framework sqlmap wpscan netcat enum4linux samba gobuster dirb dirbuster exploitdb hash-identifier hashcat hashcat-utils hashid hydra hcxtools seclists wordlists man-db burpsuite binwalk steghide chafa john python3 python3-pip python2 software-properties-common strace ltrace wget git unzip build-essential curl smbclient iproute2 iputils-ping kali-desktop-xfce net-tools dbus-x11 x11vnc xvfb psmisc ssh ftp nfs-common cargo zaproxy swaks libssl-dev libffi-dev build-essential python3-dev && \
